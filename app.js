    ...
    return h('div',{key:task.id},
      h('div',{'data-cat':cat,'data-tid':task.id,draggable:reorderOn,
        ...
        h('div',{style:{flex:1,minWidth:0}},
          ...
          (task.eventDates&&task.eventDates.length>0||task.category==='event'||cat==='event')&&h('div',{style:{fontSize:9,color:'#fb7185',marginTop:2}},'📅 '+(task.eventDates&&task.eventDates.length>0?task.eventDates.join(', '):'Sin fecha'))),
